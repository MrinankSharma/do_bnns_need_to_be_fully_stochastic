...
baselines/cifar/partially_stochastic_variational_inference.py # run subset VI
baselines/cifar/partially_stochastic_models/partially_stochastic_variational_wide_resnet.py

### scripts

## job lists: contain the actual python commands to run, but submitted using slurm
# specifically, to use those commands, you can do something like
# singularity exec --nv --bind ${DATA}:${DATA} container.sif python3.9 [joblist]
scripts/job_lists/cifar_subset_vi.txt # initial investigation into subset vi with many configs
scripts/job_lists/cifar10_subset_vi_moreseeds.txt # more seeds for CIFAR10
scripts/job_lists/cifar100_subset_vi_moreseeds.txt # more seeds for CIFAR100

scripts/generic.sh #slurm helper to dispatch jobs
scripts/dispatch_jobs.sh #slurm helper to dispatch jobs

### notebooks: for plotting and analysis
notebooks/analyse_subset_vi_from_csv.ipynb #paper plots
notebooks/analyse_subset_vi_generate_csv.ipynb # generate csv for paper plots
notebooks/subset_vi_plots_initial_investigation.ipynb #plot for many configs
