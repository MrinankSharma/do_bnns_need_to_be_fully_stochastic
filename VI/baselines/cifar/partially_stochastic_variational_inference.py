...
from partially_stochastic_models.partially_stochastic_variational_wide_resnet import (
    partially_stochastic_variational_wide_resnet,
)
...
flags.DEFINE_float(
    "stddev_init", 1e-3, "Initialization of posterior standard deviation parameters."
)
flags.DEFINE_integer(
    "num_eval_samples",
    5,
...
flags.DEFINE_float(
    "group_1_conv_l2", None, "L2 reg. coefficient for the 1st group of conv layers."
)
flags.DEFINE_float(
    "group_2_conv_l2", None, "L2 reg. coefficient for the 2nd group of conv layers."
)
flags.DEFINE_float(
    "group_3_conv_l2", None, "L2 reg. coefficient for the 3rd group of conv layers."
)
flags.DEFINE_float(
    "dense_kernel_l2", None, "L2 reg. coefficient for the kernel of the dense layer."
)
flags.DEFINE_float(
    "dense_bias_l2", None, "L2 reg. coefficient for the bias of the dense layer."
)
...
def main(argv):
    ...
    with strategy.scope():
        ...
        model = partially_stochastic_variational_wide_resnet(
            input_shape=(32, 32, 3),
            depth=28,
            width_multiplier=10,
            ...
            stochastic_layer_code=FLAGS.stochastic_layer_code,
            l2=FLAGS.l2,
    ...
    def train_step(iterator):
        ...
        def step_fn(inputs):
            ...
            with tf.GradientTape() as tape:
                ...
                l2_loss = (
                    FLAGS.l2 * 2 * tf.nn.l2_loss(tf.concat(filtered_variables, axis=0))
                )
                kl = sum(model.losses)
                kl_scale = tf.cast(optimizer.iterations + 1, kl.dtype)
                kl_scale /= steps_per_epoch * FLAGS.kl_annealing_epochs
                kl_scale = tf.minimum(1.0, kl_scale)
                kl_loss = kl_scale * kl
    ...
    for epoch in range(initial_epoch, FLAGS.train_epochs):
        ...
        if validation_dataset:
            ...
            test_step(validation_iterator, "validation", "clean", steps_per_validation)
        datasets_to_evaluate = {"clean": test_datasets["clean"]}
        if (
            FLAGS.corruptions_interval > 0
            and (epoch + 1) % FLAGS.corruptions_interval == 0
        ) or (FLAGS.corruptions_interval > 0 and epoch + 1 == FLAGS.train_epochs):
