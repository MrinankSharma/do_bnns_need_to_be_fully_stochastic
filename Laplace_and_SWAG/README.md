...
```angular2html
### Library
laplace/laplace.py # Sublayer KFAC laplace approximation

### scripts
scripts/sublayer_laplace_val.py # Laplace with holdout validation
scripts/sublayer_swag_val.py # Laplace with holdout validation
scripts/train_map.py # Train map solution

### notebooks
notebooks/plots.ipynb #paper plots

# singularity container file
singularity/singularity.def has an example singularity container file that can be used
```
To produce the results, first use `train_map.py` to produce some MAP runs. Then you can use one of the scripts!
