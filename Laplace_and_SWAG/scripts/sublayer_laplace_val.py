...
if __name__ == "__main__":
    ...
    if args.stochastic_groups_str == "00000000":
        all_res_dict["log_marginal_likelihood"] = np.nan
        all_res_dict["prior_precision"] = np.nan
        all_res_dict["test_results"] = generate_results_dict(
            all_dataloaders_dict, model, test_targets=test_targets, laplace=False
        )
    else:
        groups = [
            [0, 1],
    ...
    fname = f"{args.output_dir}/sublayer_laplace/{args.stochastic_groups_str}_s{args.seed}_val"
